...
export default function ChatPanel({ theme, persona: pagePersona }) {
  ...
  // GitHub Copilot device-flow sign-in: fetch a token without pasting a PAT.
  const fetchCopilotToken = async () => {
    setDeviceBusy(true); setDeviceMsg('Requesting a sign-in code…'); setDeviceInfo(null);
    try {
      const r = await fetch('/api/copilot/device/start', { method: 'POST' }).then((x) => x.json());
                ...
                <div className="leading-tight flex-1 min-w-0">
                  ...
                  {tokenPrompt && (
                    <div className="mt-1.5 p-2 rounded-lg" style={{ background: surface, border: `1px solid ${border}` }}>
                      <div className="text-[10px] mb-1.5 leading-snug" style={{ color: sub }}>Use your own GitHub Copilot license. Sign in with GitHub (recommended) or paste a token. It stays in your browser and is only forwarded to power your Copilot requests.</div>
                      <button onClick={fetchCopilotToken} disabled={deviceBusy}
                        className="w-full mb-1.5 px-2 py-1 rounded text-[11px] font-semibold inline-flex items-center justify-center gap-1.5 disabled:opacity-50"
                        style={{ background: theme.accent, color: dark ? '#06121f' : '#fff' }}>
                        <svg viewBox="0 0 24 24" className="w-3 h-3" fill="currentColor"><path d="M12 2C6.5 2 2 6.6 2 12.3c0 4.5 2.9 8.3 6.8 9.7.5.1.7-.2.7-.5v-1.7c-2.8.6-3.4-1.4-3.4-1.4-.5-1.2-1.1-1.5-1.1-1.5-.9-.6.1-.6.1-.6 1 .1 1.5 1 1.5 1 .9 1.6 2.4 1.1 3 .9.1-.7.4-1.1.6-1.4-2.2-.3-4.5-1.1-4.5-5 0-1.1.4-2 1-2.7-.1-.3-.4-1.3.1-2.7 0 0 .8-.3 2.7 1 .8-.2 1.7-.3 2.5-.3s1.7.1 2.5.3c1.9-1.3 2.7-1 2.7-1 .5 1.4.2 2.4.1 2.7.6.7 1 1.6 1 2.7 0 3.9-2.3 4.7-4.5 5 .4.3.7.9.7 1.9v2.8c0 .3.2.6.7.5 3.9-1.4 6.8-5.2 6.8-9.7C22 6.6 17.5 2 12 2z" /></svg>
                        {deviceBusy ? 'Waiting for GitHub…' : 'Sign in with GitHub'}
                      </button>
                      {deviceInfo && (
                        <div className="text-[10px] mb-1 leading-snug" style={{ color: text }}>
                          Enter code <b style={{ letterSpacing: '1px' }}>{deviceInfo.user_code}</b> at <a href={deviceInfo.verification_uri} target="_blank" rel="noopener noreferrer" style={{ color: theme.accent }}>{deviceInfo.verification_uri}</a>
                        </div>
                      )}
                      {deviceMsg && <div className="text-[9px] mb-1" style={{ color: sub }}>{deviceMsg}</div>}
                      <div className="flex gap-1">
                        <input type="password" value={tokenInput} onChange={(e) => setTokenInput(e.target.value)} placeholder="…or paste ghp_… / gho_…"
                        ...
                        <button onClick={() => { setTokenPrompt(false); if (devicePollRef.current) { clearInterval(devicePollRef.current); devicePollRef.current = null; } setDeviceBusy(false); }} className="px-1.5 text-[13px]" style={{ color: sub }}>×</button>
                      </div>
                    </div>
                  )}
                </div>
