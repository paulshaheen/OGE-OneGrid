...
export default function ChatPanel({ theme, persona: pagePersona }) {
  ...
  const [model, setModel] = useState(() => { try { return localStorage.getItem('pm.chat.model') || ''; } catch { return ''; } });
  const [modelOpen, setModelOpen] = useState(false);
  const [provider, setProvider] = useState(() => { try { return localStorage.getItem('pm.chat.provider') || ''; } catch { return ''; } });
  const [copilotToken, setCopilotToken] = useState(() => { try { return localStorage.getItem('pm.copilot.token') || ''; } catch { return ''; } });
  ...
  const [agentMode, setAgentMode] = useState(() => { try { return localStorage.getItem('pm.chat.agentMode') === '1'; } catch { return false; } });
